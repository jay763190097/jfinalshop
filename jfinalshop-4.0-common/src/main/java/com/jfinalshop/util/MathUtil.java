...
public final strictfp class MathUtil {
	 ...
	 * @return
	 */
	public static String getPercentage(long divisor,long dividend){
		String percentage = "";
		if(divisor!=0 && dividend!=0){
			BigDecimal b1 = new BigDecimal(Long.toString(divisor)); //除数
            BigDecimal b2 = new BigDecimal(Long.toString(dividend)); //被除数  
            double divideValue = b1.divide(b2,3,BigDecimal.ROUND_HALF_UP).doubleValue();
            BigDecimal b3 = new BigDecimal(Double.toString(divideValue));   
            BigDecimal b4 = new BigDecimal(("100"));  //百分率乘以100 
            double value = b3.multiply(b4).doubleValue();
            percentage = value + "%";
		}else{
			percentage = "0.00%";
		}
