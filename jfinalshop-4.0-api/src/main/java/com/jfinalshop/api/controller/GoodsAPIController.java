...
public class GoodsAPIController extends BaseAPIController {
	...
	public void detail() {
    	...
    	fina1 = fina.substring(0,fina.length()-1);
		}
		// 增加点击数
		...
		//渠道标识
		pGoods.setChannel(goods.getChannel());
		...
		pGoods.setProductImages(goods.getProductImages());;
		...
		List<Product> list = goods.getProducts();
		pGoods.setProducts(goods.getProducts());
		//List<SpecificationValue> list1 = goods.getProducts().get(1).getSpecificationValuesConverter();
		product1.setId(id);
		...
		if (recommends.size() < count) {
			...
			List<Goods> goodsList = goodsService.findList(null, null, null, null, null, null, null, null,channel, true, true, null, null, null, null, Goods.OrderType.salesDesc, count, pFilters, null, false);
	...
	public void checkStock(){
		Long goodsId = getParaToLong("goodsId");
		Product product = productDao.find(goodsId);
