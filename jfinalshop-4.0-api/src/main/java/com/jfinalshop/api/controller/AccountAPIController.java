...
public class AccountAPIController extends BaseAPIController {
	...
	/** 默认头像 */
	private static final String AVATAR = "/image/peiqi.jpg";
	/** redis管理 */
       ...
       /* if (memberService.usernameExists(username)) {
    	...
    	try {
			...
			if("提交成功".equals(end)){
				...
				String generate = SMSManager.getMe().generateSMS(username,SMSCODE);
				LOG.info("最新添加的短信验证码"+generate);
				 //TimerTask实现10分钟后从session中删除checkCode
  ...
  /*  public void sendLoginSms() {
    ...
    public void login() {
    	...
    	String smsCode = redisManager.get(ID);
    	//String smsCode = "123";
    	//LOG.info("redis当中的"+smsCode);
    	//String smsCode = "1234";
    	//String realPassword = rsaService.decryptParameter("password", request);
    	//rsaService.removePrivateKey(request);
    	
    	String res = SMSManager.getMe().validate(ID, code);
    	LOG.info("判定的结果是什么 0:没有提前获取 2：验证码错误"+res);
    	if ("0".equals(res)) {
...
/*		if (!DigestUtils.md5Hex(password).equals(member.getPassword())) {
        ...
        Cart cart1 = cartService.getCartKey(pMember.getId().toString());
        System.out.println("当前登录人到底是谁"+pMember.getId());
        //LOG.info("当前登录人的id是"+member1.getId());
        if(cart1!=null){
        String cartkey = cart1.getCartKey();
        Lresponse.setCartKey(cartkey);
	      //LOG.info("当前等路人的购物车key是"+cartkey);
        System.out.println("当前登录人购物车"+cartkey);
        }else{
      	  String cartkey1 = null;
      	  Lresponse.setCartKey(cartkey1); 
        }
    ...
    public void modifyInformation(){
    	UploadFile file = getFile("caseImage");
    	int gender1=0;
    	if(file!=null){
    	if(file.getFile().length()>1024 * 1024*3){
    		renderJson(new BaseResponse(Code.FAIL,"上传文件不能大于3M"));
    		return;
    	}
    	}
    	//UploadFile uploadfile = getFile("caseImage", , 1024 * 1024*3, "UTF-8");
    	Member member = getMember();
    	Long userId = member.getId();
    	Member member2 = memberService.find(userId);
    	/*String userid = getPara("username");
    	Member member = memberService.findByUsername(userid);*/
    	if(file!=null){
            LOG.info("到底有没有图片");
    	    String imagename = file.getFileName();
    	    String avator = "/upload/image/"+imagename;
     	    member.setAvatar(avator);
         }
    	LOG.info("当前是否登录呢"+member!=null);
    	String nickname = getPara("nickname",member2.getNickname());
    	System.out.println("性别"+member.getGender());
    	String gender = getPara("gender",member2.getGender().toString());
    	if(gender!=null){
    		 gender1 = Integer.parseInt(gender);
    		 member.setGender(gender1);
    	}
    	member.setNickname(nickname);
    	Member mm = memberService.update(member);
    ...
    public void information(){
    	Member member = getMember();
    	Member member3 = memberService.find(member.getId());
    	/*String userid = getPara("username");
    	Member member = memberService.findByUsername(userid);*/
    	Member member2 = new Member();
    	member2.setAvatar(member3.getAvatar());
    	member2.setNickname(member3.getNickname());
    	member2.setUsername(member3.getUsername());
    	LOG.info("当前用户的性别值"+Member.Gender.values());
    	if(Member.Gender.values()!=null){
    		
    		//LOG.info("当前用户的性别是"+member.getGenderName());
    		member2.setGender(member3.getGender());
    	}else{
    		renderJson(new DatumResponse(member));
    	}
    	renderJson(new DatumResponse(member2));
    }
}
