...
<title>${message("admin.coupon.edit")} - Powered By JFinalShop</title>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<div class="tabContent">
			<table class="input">
				<tr>
					...
					<td>
						<input type="text" name="coupon.name" class="text" value="${coupon.name}" maxlength="200" />
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<input type="text" name="coupon.prefix" class="text" value="${coupon.prefix}" maxlength="200" />
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<input type="text" id="beginDate" name="coupon.begin_date" class="text Wdate" value="[#if coupon.beginDate??]${coupon.beginDate?string("yyyy-MM-dd HH:mm:ss")}[/#if]" onfocus="WdatePicker({dateFmt: 'yyyy-MM-dd HH:mm:ss', maxDate: '#F{$dp.$D(\'endDate\')}'});" />
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<input type="text" id="endDate" name="coupon.end_date" class="text Wdate" value="[#if coupon.endDate??]${coupon.endDate?string("yyyy-MM-dd HH:mm:ss")}[/#if]" onfocus="WdatePicker({dateFmt: 'yyyy-MM-dd HH:mm:ss', minDate: '#F{$dp.$D(\'beginDate\')}'});" />
					</td>
				</tr>
				<tr>
					<th>
				...
				<tr>
					...
					<td>
						<input type="text" id="point" name="coupon.point" class="text" value="${coupon.point}" maxlength="9"[#if !coupon.isExchange] disabled="disabled"[/#if] />
					</td>
				</tr>
			</table>
		</div>
		<div class="tabContent">
			<table class="input">
				<tr>
					<td>
						<textarea id="introduction" name="coupon.introduction" class="editor" style="width: 100%;">${coupon.introduction}</textarea>
					</td>
				</tr>
			</table>
		</div>
