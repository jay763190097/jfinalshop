...
[@seo type = "index"]
	<title>[#if seo.title??][@seo.title?interpret /][#else]${message("shop.index.title")}[/#if][#if showPowered] -广蔓[/#if]</title>
...
<body>
	...
	<div class="container index">
															...
															[#list promotions as promotion]
																[#if promotion.image?has_content]
																	<a href="${base}${promotion.path}" title="${promotion.title}">
																		<img src="${base}${promotion.image}" alt="${promotion.title}" />
																	</a>
																[/#if]
															[/#list]
														</div>
								...
								[#--[@article_list article_category_id = articleCategory.id count = 6]--]
									[#--<ul class="tabContent[#if articleCategory_index > 0] hidden[/#if]">--]
										[#--[#list articles as article]--]
											[#--<li>--]
												[#--<a href="${base}${article.url}" title="${article.title}" target="_blank">${abbreviate(article.title, 40)}</a>--]
											[#--</li>--]
										[#--[/#list]--]
									[#--</ul>--]
								[#--[/@article_list]--]
		...
		[@product_category_root_list count = 6]
			[@ad_position id = 4]
				[#list adPosition as adPositions]
				[#if adPositions??]
					[#assign adIterator = adPositions.ads.iterator()/]
				[/#if]
			    [/#list]
			[/@ad_position]
			[#list productCategories as productCategory]
							...
							<div class="hotGoods">
								...
								<div>
									[#if adIterator?? && adIterator.hasNext()]
										...
										[#if ad.url??]
											<a href="${base}${ad.url}">
												<img src="${base}${ad.path}" alt="${ad.title}" title="${ad.title}" />
											</a>
										[#else]
											<img src="${base}${ad.path}" alt="${ad.title}" title="${ad.title}" />
										[/#if]
								...
								<ul>
									[#list goodsList as goods]
										[#if goods_index < 5]
											<li>
												<a href="${base}${goods.url}" title="${goods.name}" target="_blank">
													<div>
													...
													<img src="${base}/upload/image/blank.gif" data-original="${base}${goods.image!setting.defaultThumbnailProductImage}" />
												</a>
											</li>
										[#else]
											<li class="low">
