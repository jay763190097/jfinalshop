...
$().ready(function() {
	...
	var introductions = introduction.split(';');
	console.log(introductions)
	var aas = introductions[4].split('&')
	[#--var url = '${base}'+'/'+introductions[1]+'/'+introductions[2]+'/'+aas[0];--]
	var url = aas[0];
	console.log(url);
	...
	$.ajax({
		...
		success: function(data) {
			$.each(data, function (i, item) {
				var thumbnail = item.thumbnail != null ? item.thumbnail : "${base}${setting.defaultThumbnailProductImage}";
				var b = '${base}';
				var url = b+thumbnail;
			...
			<div class="span10">
				...
				<div class="productImage">
					[#if goods.productImagesConverter?has_content]
						<a href="${base}${goods.productImagesConverter[0].large}" id="zoom" rel="gallery">
							<img class="medium" src="${base}${goods.productImagesConverter[0].medium}" />
						</a>
					[#else]
							...
							[#if goods.productImagesConverter?has_content]
								[#list goods.productImagesConverter as productImage]
									<a[#if productImage_index == 0] class="current"[/#if] href="javascript:;" rel="{gallery: 'gallery', smallimage: '${productImage.medium}', largeimage: '${productImage.large}'}">
										<img src="${base}${productImage.thumbnail}" title="${productImage.title}" />
									</a>
								[/#list]
							[#else]
								<a href="javascript:;" class="current">
									<img src="${base}${setting.defaultThumbnailProductImage}" />
								</a> 
							[/#if]
						</div>
					</div>
				...
				<div id="preview" class="preview">
					...
					<div id="previewScrollable" class="scrollable">
						<div class="items">
							[#if goods.productImagesConverter?has_content]
								[#list goods.productImagesConverter as productImage]
									<img src="${base}/upload/image/blank.gif" data-original="${base}${productImage.large}" title="${productImage.title}" />
								[/#list]
							[#else]
								<img src="${base}/upload/image/blank.gif" data-original="${base}${setting.defaultLargeProductImage}" />
							[/#if]
						</div>
					</div>
					<a href="javascript:;" class="next">&nbsp;</a>
				...
				[#if goods.introduction?has_content]
					<div id="introduction" name="introduction" class="introduction">
						...
						[#noescape]
							<div><p><img id="contents" src=""/></p></div>
						[/#noescape]
					</div>
				[/#if]
				[#if goods.parameterValuesConverter?has_content]
