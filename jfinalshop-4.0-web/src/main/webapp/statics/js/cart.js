...
$(function(){
	...
	$("[data-id='del-sku']").on('click' ,function() {
		...
		$.confirms("确定要删除该购物车商品吗？",function(){
			var id = $this.parents('dd').data('id');
			$.getJSON('/wap/cart/delete.jhtml', {
				id : id
			}, function(ret){
				if(ret.status == 1) {
					$('dd[data-id='+id+']').fadeOut(400,function() {
	...
	$("[data-id='sub-btn']").on('tap',function(){
		...
		if (checked_inputs.length > 0) {
			...
			$(checked_inputs).each(function(k, v) {
				...
				skuids[i] = $(v).parents(".cart-list-item").data("skuid") > 0 ? $(v).parents(".cart-list-item").data("skuid") : [null];
		 		i++;
			});
			$(this).attr("href",'/wap/order/checkout.jhtml?skuids=' + skuids);
		} else {
			return false;
		}
	});
