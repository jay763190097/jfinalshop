$(function(){
	...
	function changeStatus(){
		...
		var url_pre = arr[1];
	 	...
	 	var currentSpecificationValueIds = new Array();
	 	var i = 0;
	 	$("dl").each(function(k){
	 		var selected = $(this).find(".selected");
	 		currentSpecificationValueIds[i] = selected.data("id") > 0 ? selected.data("id") : [null];
	 		i++;
	    });
	 	// 从数组中查找新规格的产品
	 	var product = productData[currentSpecificationValueIds.join(",")];
	 	goods.sku_id = product.id; // 赋值给旧的sku_id
	 	$(".text-org").text('￥' + product.price);
	}
