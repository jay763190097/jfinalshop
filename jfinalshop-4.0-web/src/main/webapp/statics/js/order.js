		...
		if(D.address && D.address != "null"){
			...
			$.each(D.address ,function(k, v){
				H += '<li class="address-list">'
		        	   ...
		        	   + '	</div>'
		        	   + '	<div class="edit">'
		        	   + '		<a href="?m=member&c=address&a=edit&id='+ v.id +'&referer='+ encodeURIComponent(window.location.search) +'"><img src="../statics/images/ico_21.png" /></a>'
		        	   + '	</div>'
		        	   + '</li>';
			});
